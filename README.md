...
6. 确认后通过钉钉 Webhook 推送告警（含课程名称、时间、确认关键词、证据、分析、最近转录原文），发送成功后 120 秒冷却；明确否定时不推送，调用或结果校验失败时发送标明「疑似命中（语义确认失败）」的提醒
...
- **LLM 调用次数** — 冷却期外，每个命中的片段用一次调用完成确认和语境分析；API SDK 可能自动重试失败请求。明确否定不会进入冷却，已成功发送的确认告警和疑似提醒均进入 120 秒冷却
